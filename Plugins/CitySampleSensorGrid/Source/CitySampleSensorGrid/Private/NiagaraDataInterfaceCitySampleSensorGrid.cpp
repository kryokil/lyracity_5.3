...
void UNiagaraDataInterfaceCitySampleSensorGrid::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	{
		ShaderParameters->SensorInfo = NetworkProxy->ExtractedSensorInfo
			? GraphBuilder.CreateSRV(GraphBuilder.RegisterExternalBuffer(NetworkProxy->ExtractedSensorInfo))
			: Context.GetComputeDispatchInterface().GetEmptyBufferSRV(GraphBuilder, PF_R32_UINT);
	}
