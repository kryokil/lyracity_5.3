...
void UCitySampleCrowdFunctionLibrary::SetGroomBindingType(UGroomAsset* Groom, const TArray<EGroomBindingType> LODBindingType)
{
	...
	bool bIsDirty = false;
	for (FHairGroupsLOD& HairGroupLOD : Groom->GetHairGroupsLOD())
	{
...
void UCitySampleCrowdFunctionLibrary::SetGroomInterpolationType(UGroomAsset* Groom, EGroomInterpolationType InterpolationType, bool bUseRBF)
{
	...
	bool bIsDirty = false;
	if (Groom->GetHairInterpolationType() != InterpolationType)
	{
		Groom->SetHairInterpolationType(InterpolationType);
		bIsDirty = true;
	}

	// Set RBF
	if (Groom->GetEnableGlobalInterpolation() != bUseRBF)
	{
		Groom->SetEnableGlobalInterpolation(bUseRBF);
		bIsDirty = true;
	}
...
void UCitySampleCrowdFunctionLibrary::DisableGroomPhysics(UGroomAsset* Groom)
{
	...
	bool bIsDirty = false;
	for (FHairGroupsPhysics& HairGroupsPhysics : Groom->GetHairGroupsPhysics())
	{
