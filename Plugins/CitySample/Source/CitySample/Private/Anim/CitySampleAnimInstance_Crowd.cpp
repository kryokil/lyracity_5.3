...
#include "Anim/CitySampleAnimInstance_Crowd.h"
...
#include "DrawDebugHelpers.h"
#include "Anim/CitySampleAnimSet_Locomotion.h"
#include "Anim/CitySampleAnimSet_Accessory.h"
...
void UCitySampleAnimInstance_Crowd::NativeInitializeAnimation()
{
	...
	if (Character)
	{
		...
		GetWorld()->GetTimerManager().ClearTimer(LinkAccessoryLayerCallback);
		LinkAccessoryLayerCallback = GetWorld()->GetTimerManager().SetTimerForNextTick(this, &UCitySampleAnimInstance_Crowd::LinkAccessoryLayer);
	}
}

void UCitySampleAnimInstance_Crowd::NativeUninitializeAnimation()
{
	GetWorld()->GetTimerManager().ClearTimer(LinkAccessoryLayerCallback);
...
void UCitySampleAnimInstance_Crowd::LinkAccessoryLayer()
{
	...
	LinkAccessoryLayerCallback.Invalidate();

	if(Character && GetSkelMeshComponent())
	{
