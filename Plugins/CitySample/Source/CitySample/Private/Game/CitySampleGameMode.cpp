...
void ACitySampleGameMode::BeginPlay()
{
	...
	if (CVarPerformanceMode.GetValueOnGameThread())
	{
		if (UWorld* World = GetWorld())
		{
			...
			{	
				for (const FName& DataLayerNameToDisable : DataLayersToDisableInPerformanceMode)
				{
					DataLayerManager->ForEachDataLayerInstance([DataLayerManager, &DataLayerNameToDisable](UDataLayerInstance* DataLayerInstance)
					{
						// Find DataLayerInstance for the provided label
						if (FName(DataLayerInstance->GetDataLayerShortName()) == DataLayerNameToDisable)
						{
